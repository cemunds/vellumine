...
(function () {
      ...
      for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
          if (node.nodeType === 1) {
            if (
              node.tagName === "SCRIPT" &&
              node.hasAttribute("data-sodo-search")
            ) {
              node.remove();
  ...
  class VellumineSearchElement extends HTMLElement {
    ...
    cacheElements() {
      ...
      this.commonSearches = this.shadowRoot.querySelector(
        `.${CSS_PREFIX}-common-searches`,
      );
      this.loadingState = this.shadowRoot.querySelector(
        `#${CSS_PREFIX}-loading`,
      );
    ...
    updateTheme() {
      ...
      const isDarkMode =
        this.config.theme === "light"
          ? false
          : this.config.theme === "dark" || preferDark;
    ...
    initEventListeners() {
      ...
      this.modal.addEventListener("click", (e) => {
        if (
          e.target === this.modal ||
          e.target.classList.contains(`${CSS_PREFIX}-backdrop`)
        ) {
          this.closeModal();
        }
      });
    ...
    attachCommonSearchListeners() {
      const container = this.shadowRoot.querySelector(
        `#${CSS_PREFIX}-common-searches-container`,
      );
      if (!container) return;
    ...
    closeModal() {
      ...
      if (
        this.activeElement &&
        typeof this.activeElement.focus === "function"
      ) {
    ...
    async handleSearch(query) {
      ...
      if (!query) {
        ...
        if (this.commonSearches)
          this.commonSearches.classList.remove(`${CSS_PREFIX}-hidden`);
        if (this.emptyState)
          this.emptyState.classList.add(`${CSS_PREFIX}-hidden`);
      ...
      if (this.commonSearches)
        this.commonSearches.classList.add(`${CSS_PREFIX}-hidden`);
      if (this.hitsList) this.hitsList.classList.remove(`${CSS_PREFIX}-hidden`);
      if (this.loadingState)
        this.loadingState.classList.remove(`${CSS_PREFIX}-hidden`);
      if (this.emptyState)
        this.emptyState.classList.add(`${CSS_PREFIX}-hidden`);
      ...
      try {
        ...
        if (results.hits.length === 0) {
          if (this.emptyState)
            this.emptyState.classList.remove(`${CSS_PREFIX}-hidden`);
          if (this.hitsList) {
          ...
          .map((hit) => {
            ...
            const getHighlightedTitle = (fieldName, fallback) => {
              if (
                ...
                return (
                  hit.highlight[fieldName].snippet ||
                  hit.highlight[fieldName].value ||
                  fallback
                );
              }
              return fallback;
            ...
            const title =
              getHighlightedTitle("title", hit.document.title) ||
              this.t("untitledPost");
            const excerpt =
            ...
            const visibilityBadge =
              visibility === "public"
                ? ""
                : `<p class="${CSS_PREFIX}-result-badge">${visibility}</p>`;
    ...
    getSearchParameters() {
      const fields =
        Object.keys(this.config.searchFields || {}).length > 0
    ...
    navigateResults(direction) {
      const results = [
        ...this.shadowRoot.querySelectorAll(
          `.${CSS_PREFIX}-result-link, .${CSS_PREFIX}-common-search-btn:not(.${CSS_PREFIX}-hidden)`,
        ),
      ].filter(
        (el) =>
          el.offsetParent !== null && !el.closest(`.${CSS_PREFIX}-hidden`),
      );
      ...
      results.forEach((result) =>
        result.classList.remove(`${CSS_PREFIX}-selected`),
      );
